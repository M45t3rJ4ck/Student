...
class Animal:
    ...
    # Can be used to cast our object as a string type(self).__name__ returns the class name
    def __str__(self):
...
class Reptile(Animal):
    ...
    # Operator overloading isn't necessary in Python because Python allows you to enter unknown numbers of values
...
def main():
    ...
    # Polymorphism in Python works differently from otherclanguages in that functions accept any object and expect
    # that object to provide the needed method. This isn't something to dwell on. Just know that if you call on a
    # method for an object that the method just needs to exist for that object to work. Polymorphism is a big deal in
    # other languages that are statically typed (type is defined at declaration) but because Python is dynamically
    # typed (type defined when a value is assigned) it doesn't matter as much.
...
# ---------- MAGIC METHODS ----------
# Magic methods are surrounded by double underscores. We can use magic methods to define how operators like +, -, *,
# /, ==, >, <, etc. will work with our custom objects. Magic methods are used for operator overloading in Python
# __eq__ : Equal
# __ne__ : Not Equal
# __lt__ : Less Than
# __gt__ : Greater Than
